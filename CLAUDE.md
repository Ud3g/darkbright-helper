...
- **Tray thread** (`platform/windows/tray.rs`): tray icon + context menu; uses a request/response message (`TrayMenuOpening { reply_tx }`) to fetch live monitor status *and* the current hotkey bindings when the menu opens — the usage rows are no longer frozen at spawn, since the settings window can rebind hotkeys while the app runs.
