...
Tests live in-module (`#[cfg(test)]`) under `src/` — the controller's, by far the largest, sits beside its module in `src/core/controller/tests.rs` — plus integration tests in `tests/` (`ddc_test.rs`, `hotkey_test.rs`, `single_instance_test.rs`); the crate-root example in `src/lib.rs` is a real doctest. Hardware-dependent DDC/refresh behavior is verified **manually** — see the "Integration Testing" section of `docs/architecture.md`; changing such a path updates its procedure in the same PR. (CI does cover the settings window's layout itself: `settings/plan.rs`'s tests plan every language at four DPIs, with both a development and a release version string (a release build's window can be narrower), and assert no overflow, overlap, or out-of-bounds window size — manual verification there is limited to real rendering, theming and DPI-change timing.)
...
- `src/core/` — platform-agnostic: `brightness.rs` (adjustment math), `config.rs` (JSON config + validation, `restore_defaults`, `overlay_dirty` for merge-on-save), `controller.rs` (message-driven orchestration behind the platform seams), `edid.rs` (EDID → `MonitorId` parsing), `i18n.rs` + `i18n/` (the user-visible strings: `Strings` and the language logic in `i18n.rs`, one table per language in `i18n/<tag>.rs` — log text and the config wire formats stay out of it), `logfile.rs` (rolling file sink), `panic_hook.rs` (panic logging), `reconcile.rs` (refresh/respawn/save-debounce/rebind-ack timing constants), `state.rs` (state, messages, `MonitorId`, display-name generation).
...
- `src/platform/windows/` — all Win32 FFI: `ddc.rs`, `ddc_worker.rs`, `hotkey.rs`, `osd.rs`, `osd_render.rs` (private), `overlay.rs`, `power.rs`, `single_instance.rs`, `theme.rs` (dark-mode opt-in for the tray menu and, via `system_prefers_dark()`, the settings window), `tray.rs`, `autostart.rs` (`HKCU\Run`-backed "Start with Windows"), `config_store.rs` (`WindowsConfigStore`, the `ConfigStore` seam — save-with-merge onto externally-edited config), `locale.rs` (`WindowsLanguageSource`, the `LanguageSource` seam — the OS's UI-language list via `GetUserPreferredUILanguages`); `settings/` (directory module: `mod.rs`, `layout.rs` declarative control table, `measure.rs` the `TextMeasure` text-measurement port (GDI-backed for the live window, fakeable for tests), `plan.rs` the layout planner that turns the control table plus measurements into positioned rectangles, `window.rs` window/wiring/`SettingsSinkImpl`, `capture.rs` hotkey-capture control, `dark.rs` dark-mode painting) — the settings dialog, on its own thread; plus `mod.rs`, which is not a bare gate — it holds `CursorLocator` (the `MonitorLocator` impl), the message-box helpers, the embedded app-icon loader the settings window uses, and the public re-exports the binary imports.
