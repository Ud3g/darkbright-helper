...
- `src/platform/windows/` — all Win32 FFI: `ddc.rs`, `ddc_worker.rs`, `hotkey.rs`, `osd.rs`, `osd_render.rs` (private), `overlay.rs`, `power.rs`, `single_instance.rs`, `theme.rs` (dark-mode opt-in for the tray menu), `tray.rs`; plus `mod.rs`, which is not a bare gate — it holds `CursorLocator` (the `MonitorLocator` impl), the message-box helpers, and the public re-exports the binary imports.
